...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, target=False, eta=1.0,
                only_det=False, batch_size=1, only_domain=False, test=False):
        ...
        if not only_det:
            ...
            if only_domain or target:
                return d_pixel, domain_p
        if only_det and (not target) and self.lc:
            _, feat_pixel = self.netD_pixel(base_feat1.detach())
        if only_det and (not target) and self.gc:
            _, feat = self.netD(base_feat.detach())
        ...
        # print(bbox_pred.shape, return_feat.shape)  # [1, 300, 28], [300, 2048]
        if test:
            return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, d_pixel, domain_p, return_feat
        else:
