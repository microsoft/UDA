...
class netD_pixel(nn.Module):
    def __init__(self, context=False):
        ...
        self.conv1 = Conv2dML(256, 256, kernel_size=1, stride=1,
                              padding=0, bias=False)
        self.conv2 = Conv2dML(256, 128, kernel_size=1, stride=1,
                              padding=0, bias=False)
        self.conv3 = Conv2dML(128, 1, kernel_size=1, stride=1,
                              padding=0, bias=False)
        self.context = context
        self._init_weights()
...
class resnet(_fasterRCNN):
    ...
    def _init_modules(self):
        resnet = resnet101()
        if self.pretrained:
            logging.info("==> Loading pretrained weights from %s | resnet101" % self.model_path)
