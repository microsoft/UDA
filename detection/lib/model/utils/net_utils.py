...
def vis_detections(im, class_name, dets, thresh=0.8, color=(0, 0, 0)):
    """Visual debugging of detections."""
    for i in range(np.minimum(10, dets.shape[0])):
        bbox = tuple(int(np.round(x)) for x in dets[i, :4])
        score = dets[i, -1]
        if score > thresh:
            cv2.rectangle(im, bbox[0:2], bbox[2:4], color, 3)
            cv2.putText(im, '%s: %.2f' % (class_name, score), (bbox[0] + 5, bbox[1] + 20), cv2.FONT_HERSHEY_PLAIN,
                        2.0, color, thickness=2)
    return im
