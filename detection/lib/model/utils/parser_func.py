...
def parse_args():
    ...
    parser.add_argument('--meta_lr',    type=float, default=1.0)

    # my test
    parser.add_argument('--save_feat',  action='store_true', dest='save_feat')
...
def set_dataset_args(args, test=False):
    if not test:
        ...
        if args.dataset == "pascal_voc":
            ...
            args.imdb_name = 'voc_water_2007_trainval+voc_water_2012_trainval'
            args.imdbval_name = 'voc_water_2012_trainval'
            args.set_cfgs = ['ANCHOR_SCALES', '[8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES',
                             '20']
