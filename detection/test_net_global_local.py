...
from da_utils.utils import create_logger
...
if __name__ == '__main__':
    ...
    logging.info("==> loaded from {}".format(args.load_name))
    ...
    for i in range(num_images):
        ...
        if args.save_feat:
            ...
            rois_label, d_pred, _, roi_feats = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, batch_size=1, test=True)
            rois_feats_list = []
        else:
            det_tic = time.time()
            rois, cls_prob, bbox_pred, \
            rpn_loss_cls, rpn_loss_box, \
            RCNN_loss_cls, RCNN_loss_bbox, \
            rois_label, d_pred, _ = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, batch_size=1)
        ...
        pred_boxes /= data[1][0][2].item()  # [300, 28]

        scores = scores.squeeze()  # [300, 7]
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            if inds.numel() > 0:
                ...
                keep = nms(cls_dets, cfg.TEST.NMS)  # [N, 1]
                cls_dets = cls_dets[keep.view(-1).long()]  # [N, 5]
                all_boxes[j][i] = cls_dets.cpu().numpy()
                # if args.save_feat:
                #     rois_feats_list.append(roi_feats[keep.view(-1).long(), :].data.cpu().numpy())
            else:
        ...
        if i % 50 == 0:
            print('    I: {:d}/{:d} | t_det: {:.3f}s | t_nms: {:.3f}s'
